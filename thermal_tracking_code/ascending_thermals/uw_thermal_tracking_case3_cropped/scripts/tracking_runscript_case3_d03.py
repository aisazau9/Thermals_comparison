...
import matplotlib
matplotlib.use('Agg')
import WRF_3Dtracing_functions as trace
import pdb
import time
...
path   = '/outputs_CASE3_new_budgets/splitted/'      # model output data location
...
def track_thermals(dx, YY0, MM0, DD0, hr0, min0, nt, x0, y0, i0, j0, nx, ny, nxkm, nykm,
    ...
    start = time.time()
    # Open final grid
    with open('/outputs_CASE3_new_budgets/grid_tracking_cropped_new/All_grid.pkl', 'rb') as f:
        domain = dill.load(f)
