...
import os
os.chdir('/uw_thermal_tracking_case2_cropped/scripts/')
...
path   = '/outputs_CASE2_new_budgets/splitted/'      # model output data location
...
downdrafts = False   # set to True if tracking downdrafts instead of rising thermals.
#*************************************************************
...
def track_thermals(dx, YY0, MM0, DD0, hr0, min0, nt, x0, y0, i0, j0, nx, ny, nxkm, nykm,
    ...
    start = time.time()
    # Open final grid
    with open('/outputs_CASE2_new_budgets/grid_tracking_cropped_new/All_grid.pkl', 'rb') as f:
        domain = dill.load(f)
